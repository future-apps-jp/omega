...
\begin{corollary}[Substrate Hypothesis---Conditional Inference]
\label{cor:substrate}
\textbf{If} our universe is quantum-mechanical (empirically verified) \textbf{and} the Principle of Algorithmic Naturalness holds, \textbf{then} the computational substrate of our universe is quantum-native ($\UQ$), not classical ($\UC$).
\end{corollary}

\begin{remark}[Logical Status of the Substrate Hypothesis]
\label{rem:substrate-status}
The Substrate Hypothesis is \emph{not} a theorem derivable from mathematics alone, but a \textbf{conditional inference} based on two premises:
\begin{enumerate}
    \item \textbf{Empirical premise}: Our universe exhibits quantum mechanics (verified to extraordinary precision).
    \item \textbf{Methodological premise}: The Principle of Algorithmic Naturalness---among computational substrates capable of hosting a given universe, the one with minimal description length is preferred.
\end{enumerate}
The second premise is a quantified form of Occam's razor, consistent with Solomonoff induction and minimum description length (MDL) inference. It is not logically necessary but is methodologically well-motivated. The $2^{878}$ probability ratio (Section~\ref{sec:experiment}) quantifies how strongly Algorithmic Naturalness prefers $\UQ$ over $\UC$ for a quantum universe.

We emphasize that this is \emph{inference to the best explanation}, not deductive proof. A classical substrate $\UC$ remains logically possible but is disfavored by a factor of $\sim 2^{878}$---roughly the number of atoms in $10^{264}$ observable universes.
\end{remark}

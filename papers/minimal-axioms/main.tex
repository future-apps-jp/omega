...
\begin{itemize}
    ...
    \item \texttt{perm\_embeds\_in\_Sp}: Proven (depends on \texttt{perm\_matrix\_orthogonal})
    \item \texttt{no\_superposition\_from\_perm}: Proven (depends on \texttt{perm\_maps\_basis})
\end{itemize}

The auxiliary lemmas (\texttt{perm\_matrix\_orthogonal}, \texttt{perm\_maps\_basis}) are axiomatized as standard results in linear algebra, and can be fully formalized with additional MathComp development.

...
This work is motivated by the ``Unified Omega Hypothesis'' \cite{kohashiguchi2025}, which proposes a philosophical framework linking Chaitin's halting probability $\Omega$, Teilhard de Chardin's Omega Point, and computational cosmology. That hypothesis raises a natural question: if the universe is fundamentally computational, what is the minimal computational substrate from which physical laws---including quantum mechanics---could emerge?
...
\begin{itemize}
    ...
    \item We provide open-source implementations for reproducibility\footnote{Available at: \url{https://github.com/future-apps-jp/omega/}}
\end{itemize}
...
This work stands on the shoulders of giants in multiple fields. We are deeply indebted to the foundational contributions of:

\textbf{In combinatory logic}: Haskell Curry and Moses Sch\"onfinkel, whose minimal computational formalism provided the substrate for this investigation; and to the lambda calculus tradition established by Alonzo Church.

\textbf{In quantum foundations}: Lucien Hardy, whose axiomatic derivation of quantum theory from reasonable postulates inspired our approach; Rafael Sorkin, whose quantum measure theory provided the key experimental criterion ($I_2 \neq 0$, $I_3 = 0$); and Giulio Chiribella, Giacomo Mauro D'Ariano, and Paolo Perinotti for their information-theoretic perspective.

\textbf{In the physics of computation}: Rolf Landauer, whose principle connecting information erasure to thermodynamics motivated our analysis of the K combinator; Charles Bennett for reversible computation; and Gregory Chaitin, whose work on algorithmic information theory and the halting probability $\Omega$ provided the philosophical motivation for this research program.

\textbf{In digital physics}: Stephen Wolfram, whose ambitious Physics Project rekindled interest in computational approaches to fundamental physics and highlighted the challenge of deriving complex amplitudes that this paper addresses.

\textbf{In geometric phase}: Michael Berry and Yakir Aharonov, whose work on geometric phases in quantum mechanics inspired our holonomy-based approach.

We also acknowledge the intellectual heritage of Pierre Teilhard de Chardin, whose concept of the Omega Point, though theological in origin, suggested deep connections between computation, consciousness, and cosmological finality that motivated the broader research program within which this investigation is situated.

Finally, we thank the open-source community for providing the computational tools that made this systematic investigation possible.
...
\begin{figure}[h]
\centering
\includegraphics[width=0.85\textwidth]{figures/figure1_diamond.pdf}
\caption{Multiway graph for $(K\,a\,b)\,(K\,c\,d)$. Two paths exist: left-first and right-first K-reductions. Both terminate at $a\,c$. This creates a loop structure where holonomy can be computed. Blue boxes indicate the initial expression, green boxes indicate normal forms (terminal states), and edge colors distinguish K-reductions.}
\label{fig:multiway-simple}
\end{figure}

\begin{figure}[h]
\centering
\includegraphics[width=0.85\textwidth]{figures/figure2_s_combinator.pdf}
\caption{Multiway graph for $S\,(K\,a)\,(K\,b)\,c$. After the initial S-reduction (blue edge), two K-reductions are possible in either order. The path space has a diamond structure: both paths share the same number of reductions (1 S + 2 K), yielding zero holonomy under the ``linear'' formula. This structure is characteristic of confluent rewriting systems.}
